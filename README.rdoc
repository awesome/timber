...
Inside the +Timber.registration+ block, you can +subscribe+ to as many <tt>controller#action</tt> events as you like. In order to create an activity record, however, each subscription should include a call to <tt>Timber::Activity.create</tt>:
  ...
  Timber::Activity.create(
    ...
    # But keep in mind that ActiveRecord doesn't make it easy to query serialized. See:
    # http://stackoverflow.com/questions/9814622
    parameters: { author_name: post.author.name }
  )
...
This module provide a +publish_notification+ method which allows you to publish a notification, specifying a controller, action, and request params. You can find {example usage in the test suite}[https://github.com/scholastica/timber/blob/master/spec/timber/timber_spec.rb].
