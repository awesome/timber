...
Inside the +Timber.registration+ block, you can +subscribe+ to as many <tt>controller#action</tt> events as you like. Also, notice that you have access to your applications custom url helpers (e.g. +author_path+) as well as built-in helpers (e.g. +link_to+). You can also access the current user off the event object (+event.current_user+).
...
In order to create an activity record, each subscription should include a call to <tt>event.log</tt>:
  ...
  event.log(
    ...
    # The key string used to lookup the text template in +timber.en.yml+. If omit this option, it will
...
The +parameters+ hash should be used to store whatever data necessary to render your activity message. Following this pattern will prevent slow page loads (especially if you're preforming joins, etc). For example, let's say you're logging the creation of a post:
...
=== Keep view rendering logic out of +timber.yml+.
...
<i>This project uses {MIT-LICENSE}[https://github.com/scholastica/timber/blob/master/MIT-LICENSE].</i> http://docs.scholastica.s3.amazonaws.com/github_scholastica_footer_logo.png
